...
# The function to validate the imm assays is available in the olink_qc.R file
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#' @title Check IMM metadata samples file
#'
#' @description Checks whether the metadata_sample file for IMM assays follows the required guidelines.
#' @param df (data.frame) IMM metadata samples data
...
#' @examples
#' \dontrun{
#' check_metadata_samples_imm(df = metadata_sample_named_CK_plasma)
#' }
#' @export
check_metadata_samples_imm <- function(df,
                                       return_n_issues = FALSE,
                                       verbose = TRUE) {
...
} # End of check_metadata_samples_imm

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#' @title Load and Process IMM Batch Data
#'
#' @description
#' This function loads IMM batch data from the specified input directory.
#' It performs quality checks on the data and loads specific files related to
#' IMM data, including metadata for analytes and samples, and the results file.
...
#' containing IMM batch data to be loaded and processed.
...
#' metadata of samples (`m_s`), and IMM results (`r_o`). If certain files
#' are not available, the corresponding entries in the list will be `NULL`.
#' @examples
#' \dontrun{
#' list_of_df <- load_imm_batch(input_results_folder = "/path/to/PROCESSED_YYYYMMDD/")
#' }
#' @export
load_imm_batch <- function(input_results_folder,
                           verbose = TRUE) {
  ...
  # Perform validation using validate_imm
  total_issues <- validate_imm(input_results_folder = input_results_folder,
                               cas = "duke",
  ...
  # Load IMM data ----
  if (verbose) message("# LOAD IMM BATCH")
...
#' Validate IMM Assay Data
#'
#' This function validates IMM assay data based on a set of criteria, including
...
#'
#' @param input_results_folder A string representing the path to the folder
#'        containing IMM assay results to be validated.
...
#' @examples
#' \dontrun{
#' validate_imm("/path/to/results", cas = "broad_rg", return_n_issues = TRUE)
#' }
#' @export
validate_imm <- function(input_results_folder,
                         cas,
                         return_n_issues = FALSE,
                         full_report = FALSE,
                         f_proof = FALSE,
  ...
  analyte_name = plot_basic_imm_qc = sample_id = sample_order = value = NULL
  ...
  if (purrr::is_empty(input_folder_short)) {
    ...
    if (verbose) message("/full/path/to/folder/HUMAN/T02/IMM_CK/BATCH1_20230620/PROCESSED_20230620")
    stop("Input folder not according to guidelines")
  }
  
  if (verbose) message("# IMM Assay QC Report\n\n")
  ...
  if (f_ms) {
    m_s_f <- lista$filename
    m_s <- lista$df
    ic_m_s <- check_metadata_samples_imm(df = m_s,
                                         return_n_issues = TRUE,
                                         verbose = verbose)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#' @title Write IMM Data Release
#'
#' @description Write out IMM data releases. This function doesn't check whether
...
#' }
#' @export
write_imm_releases <- function(input_results_folder,
  ...
  # Load IMM datasets ----
  imm_df <- load_imm_batch(input_results_folder = input_results_folder, verbose = FALSE)
  ...
  # Decide on the OMICS folder for IMM data
  # IMM assays are typically classified under "clinical-chemistry"
  omics_folder <- "clinical-chemistry"
  
  # Construct the output folder path
  output_folder <- file.path(folder_root, folder_name, phase_folder_release, omics_folder, folder_tissue, folder_assay)
  ...
  write.table(imm_df$m_a, metadata_analyte, row.names = FALSE, sep = "\t", quote = FALSE)
  write.table(imm_df$m_s, metadata_samples, row.names = FALSE, sep = "\t", quote = FALSE)
  write.table(imm_df$r, results, row.names = FALSE, sep = "\t", quote = FALSE)
