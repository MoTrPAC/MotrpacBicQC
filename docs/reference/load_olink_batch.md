...
to Olink data, including metadata for proteins and samples, and the
...
- input_results_folder:
  ...
  A string representing the path to the folder containing Olink batch
  data to be loaded and processed.
...
A list containing data frames for metadata of proteins (m_p), metadata
of samples (m_s), and Olink results (r_o). If certain files are not
